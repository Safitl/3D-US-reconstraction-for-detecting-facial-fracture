...
if __name__ == "__main__":
    ...
    # Seed everything (model init, dataloader shuffle, etc.) for reproducible
    # runs. NOTE: the v2/v3 reported runs were trained BEFORE this was added, so
    # they are not reproducible from here; this only fixes future runs.
    pl.seed_everything(config.dataset.get("seed", 42), workers=True)
    ...
    trn_ds, _ = dataset_obj.from_path(config.dataset)

    # Use Test/ folder as val set if it exists, otherwise fall back to internal split
    test_path = Path(config.dataset.root) / "Test" / "gts"
    if test_path.exists() and any(test_path.iterdir()):
        val_ds = dataset_obj.from_path(config.dataset, mode="Test")
        logger.info("Using Test/ folder as validation set.")
    else:
        _, val_ds = dataset_obj.from_path(config.dataset)
        val_ds = torch.utils.data.Subset(val_ds, range(0, min(100, len(val_ds))))
        logger.info("No Test/ folder found — using internal Train/ split as validation.")

    trn_dl = get_dataloaders(config.dataset, trn_ds)
    val_dl = torch.utils.data.DataLoader(
        val_ds, batch_size=1, shuffle=False,
        num_workers=config.dataset.num_workers, drop_last=False,
    )
    ...
    # wandb_logger.watch(model, log_graph=False)  # disabled — gradient hooks waste VRAM
    csv_logger = CSVLogger(save_dir="./logs", name="csv_metrics")
    _script_dir = Path(__file__).resolve().parent
    _raw_save_path = config.training.get("save_path")
    _ckpt_dir = (
        _script_dir / _raw_save_path if _raw_save_path is not None
        else _script_dir / "checkpoints"
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,  # disk is tight — keep only the single best epoch (+ last for resume)
        mode="max",
    )
